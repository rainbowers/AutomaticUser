...
class exapi():
    def __init__(self):
        self.ip = Params['ExchangeConfig']['host']
        self.user = Params['ExchangeConfig']['user']
        self.password = Params['ExchangeConfig']['password']
        self.wsman = WSMan(self.ip, port=80, path="/powershell/",ssl=False,username=self.user, password=self.password,auth="basic",encryption='never')
...
# alias 邮箱别名
# database 数据库
def EnableRemoteMailboxhigh(identity,alias=None,remoteroutingaddress=None,**kwargs):
    try:
        exchange = exapi()
        if alias:
            exchange.connection('Enable-RemoteMailbox',identity=identity,alias=alias,RemoteRoutingAddress=remoteroutingaddress,**kwargs)
        else:
            exchange.connection('Enable-RemoteMailbox',identity=identity,RemoteRoutingAddress=remoteroutingaddress,**kwargs)
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
...
# 查询用户邮箱，添加|fl参数表示显示详情
#identity 群组唯一标识（必须）
def GetMailUser(identity,**kwargs):
    try:
        exchange = exapi()
        exchange.connection('Get-MailUser', identity="%s"%identity, **kwargs)
...
# Disable-MailUser
# https://docs.microsoft.com/en-us/powershell/module/exchange/disable-mailuser?view=exchange-ps
# 禁用用户邮箱
#identity 用户唯一标识（必须）
def DisableMailbox(identity,**kwargs):
    try:
        exchange = exapi()
        kwargs.update(confirm=False)
        exchange.connection('Disable-Mailbox', identity=identity,**kwargs)
        exchange.Serializationmessage()
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
...
# https://docs.microsoft.com/en-us/powershell/module/exchange/Disable-RemoteMailbox?view=exchange-ps
# 禁用用户邮箱
#identity 群组唯一标识（必须）
def DisableRemoteMailUser(identity,**kwargs):
    try:
        exchange = exapi()
        kwargs.update(confirm=False)
        exchange.connection('Disable-RemoteMailbox', identity=identity, **kwargs)
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
...
#identity 群组唯一标识（必须）
#alias 别名
def EnableDistributionGroup(identity,**kwargs):
    try:
        exchange = exapi()
        # exchange.connection('enable-DistributionGroup', identity=identity,alias=alias, **kwargs)
        exchange.connection('enable-DistributionGroup', identity=identity, **kwargs)
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
...
def NewMailContacthight(identity,ExternalEmailAddress,**kwargs):
    try:
        exchange = exapi()
        exchange.connection('New-MailContact',identity=identity ,ExternalEmailAddress=ExternalEmailAddress,**kwargs)
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
...
def AddDistributionGroupMember(identity,mailaddress):
    try:
        exchange = exapi()
        exchange.connectionscript('Add-DistributionGroupMember', identity=identity,member=mailaddress)
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
...
def GetQueue(**kwargs):
    try:
        exchange = exapi()
        exchange.connection('get-queue',**kwargs)
        return  exchange.returnfuction()
    except Exception as e:
        return  {'isSuccess': False, 'count': 0, 'msg': str(e), 'code': 201, 'message': ''}
